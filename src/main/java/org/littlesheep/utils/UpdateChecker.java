...
public class UpdateChecker {
    ...
    public UpdateChecker(paytofly plugin) {
        this.plugin = plugin;
        this.exceptionHandler = plugin.getExceptionHandler();
    }
    ...
    public void checkForUpdates() {
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            ExceptionHandler.SafeResult<Void> result = exceptionHandler.executeSafely(
                "checkForUpdates", () -> {
                    ...
                    if (connection.getResponseCode() == HttpURLConnection.HTTP_OK) {
                        ...
                        throw new java.io.IOException("HTTP响应代码: " + connection.getResponseCode());
                    }
                    return null;
                }
            );
            ...
            if (!result.isSuccess()) {
                plugin.getLogger().warning("检查更新失败: " + result.getException().getMessage());
            }
        });
    }
} 